#ifndef CLASE_PERSONAJE_ENEMIGO_5_H_INCLUDED
#define CLASE_PERSONAJE_ENEMIGO_5_H_INCLUDED

#define FIRE_ENEMIGO_5 6

class ENEMIGO_5: public PERSONAJE{
    protected:
        ...
        bool asechar_activado;

        FLECHA carcaj[FIRE_ENEMIGO_5];
    ...
    public:
        ...
        ///Rutina de ataque:
        int realizar_ataque(MAPA &mapa, int x_guia, int y_guia, int x_juego, int y_juego, int escudo);
        bool verificar_ataque(int x_jugador, int y_jugador, int x_enemigo, int y_enemigo);
        void Lanzar_flechas(MAPA &mapa, int x_guia, int y_guia, int x_juego, int y_juego);
        bool Lanzar_flecha(MAPA &mapa);
        ...
        void Graficar_accion_MOBs_caminar_asechar();
...
ENEMIGO_5::ENEMIGO_5(){
    ...
    frames_animacion_ataque.sets_tiempo(3);
...
///Reiniciar MOBs:
void ENEMIGO_5::Reiniciar_MOBs(MAPA &mapa, int pocicion_x_no_aseptable, int pocicion_y_no_aseptable){
    ...
    asechar_activado = false;

}

///--- Rutinas del enemigo 2:
void ENEMIGO_5::Rutinas(MAPA &mapa){
...
void ENEMIGO_5::rutina_de_movimiento(MAPA &mapa){
    ...
    if(!frame_estatico.gets_cont_bool()){
        ...
        if(!retraso_movimiento.control_bool_invertido()){
            ...
            if(cuadrante!=0){
                ...
                asechar_activado = true;

                switch(cuadrante){                     /// SI Y SOLO SI HAY ESPACIO PARA MOVERSE, SE MOVERA EN DIRECCION DEL SWITCH
...
void ENEMIGO_5::realisar_movimiento(MAPA &mapa,  int x_guia, int y_guia, int x_mapa, int y_mapa){
...
int ENEMIGO_5::asechar(MAPA &mapa){
...
void ENEMIGO_5::mover_MOB(MAPA &mapa){
...
int ENEMIGO_5:: buscar_cuadrante(int x, int y, int i, int j){   ///"*" SIGNIFICA QUE ESTA CHEQUEADA LA LOGICA
...
///Resibir daño.
void ENEMIGO_5::restar_vida(int dano){
...
int ENEMIGO_5::gets_pocicion_x_guia(){
    return PERSONAJE::gets_pocicion_x_guia();
}

int ENEMIGO_5::gets_pocicion_y_guia(){
    return PERSONAJE::gets_pocicion_y_guia();
}

int ENEMIGO_5::gets_pocicion_x_juego(){
    return PERSONAJE::gets_pocicion_x_juego();
}

int ENEMIGO_5::gets_pocicion_y_juego(){
    return PERSONAJE::gets_pocicion_y_juego();
}

///Sets:

void ENEMIGO_5::sets_pocicion_completa(int x_guia, int y_guia, int x_juego, int y_juego){
...
void ENEMIGO_5::sets_pocicion_x_guia(int x_guia){
    PERSONAJE::sets_pocicion_x_guia(x_guia);
}

void ENEMIGO_5::sets_pocicion_y_guia(int y_guia){
    PERSONAJE::sets_pocicion_y_guia(y_guia);
}

void ENEMIGO_5::sets_pocicion_x_juego(int x_juego){
    PERSONAJE::sets_pocicion_x_juego(x_juego);
}

void ENEMIGO_5::sets_pocicion_y_juego(int y_juego){
    PERSONAJE::sets_pocicion_y_juego(y_juego);
}


#endif // CLASE_PERSONAJE_ENEMIGO_5_H_INCLUDED
